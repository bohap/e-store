(function() {
	...
	OrderCreateController.$inject = ['items', '$uibModalInstance', 'BookUtil', 'Basket', 'ToastrNotify'];

	function OrderCreateController(items, $uibModalInstance, BookUtil, Basket, ToastrNotify) {
		var vm = this;
		var tabsSize = 3;
		...
		function create() {
			...
			function onOrderCreatingSuccess() {
				vm.sending = false;
				ToastrNotify.success("Order is created successfully", "Order Created");
				$uibModalInstance.close('created');
			}

			function onOrderCreatingFailed() {
				vm.sending = false;
				ToastrNotify.error("Error occurred while creating the order. Plase try again", "Failed");
			}
		}
	}
})();
